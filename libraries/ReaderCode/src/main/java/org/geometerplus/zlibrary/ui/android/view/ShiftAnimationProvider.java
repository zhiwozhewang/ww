...
class ShiftAnimationProvider extends SimpleAnimationProvider {
	...
	protected void drawInternal(Canvas canvas) {
		...
		if (myDirection.IsHorizontal) {
			...
			canvas.drawBitmap(getBitmapTo(), dX > 0 ? dX - myWidth : dX + myWidth, mMarginTop, myPaint);
			canvas.drawBitmap(getBitmapFrom(), dX, mMarginTop, myPaint);

            // cancel divider with two pages
//			if (dX > 0 && dX < myWidth) {
...
//			}
			...
			canvas.drawBitmap(getBitmapTo(), mMarginTop, dY > 0 ? dY - myHeight : dY + myHeight, myPaint);
			canvas.drawBitmap(getBitmapFrom(), mMarginTop, dY, myPaint);
//			if (dY > 0 && dY < myHeight) {
