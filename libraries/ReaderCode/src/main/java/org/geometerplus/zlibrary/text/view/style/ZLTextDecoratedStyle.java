...
public abstract class ZLTextDecoratedStyle extends ZLTextStyle {
    ...
    myFirstLineIndent = 0; //固定首行缩进为0
    if (fbReaderApp != null) {
      if (fbReaderApp.getType() == FBReader.TYPE_BOOK) {
        //myFirstLineIndent = getFirstLineIndentInternal(metrics, myFontSize) + 50; //固定首行缩进为0
      }
        //FIXME 段首缩进两个字符(2015.08.04)
//        myFirstLineIndent = getFirstLineIndentInternal(metrics, myFontSize) + 0;
    }
	}
