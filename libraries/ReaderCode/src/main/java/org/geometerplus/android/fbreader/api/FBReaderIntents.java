...
public abstract class FBReaderIntents {
  ...
  static  {
    String defaultPackageName = ReaderApp.getInstance().getDefaultPackageName();
    if (!TextUtils.isEmpty(defaultPackageName)) {
      DEFAULT_PACKAGE = defaultPackageName;
    } else {
      DEFAULT_PACKAGE = "com.loopeer.android.apps.lreader";
    }
  }
