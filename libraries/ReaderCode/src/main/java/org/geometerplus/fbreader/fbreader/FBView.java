...
//		}
	...
	public boolean onFingerPress(int x, int y) {
        ...
        final int headerHeight = getHeaderArea() == null ? 0 : getHeaderArea().getHeight();
        final ZLTextSelectionCursor cursor = findSelectionCursor(x, y - headerHeight, MAX_SELECTION_DISTANCE);
		if (cursor != ZLTextSelectionCursor.None) {
			myReader.runAction(ActionCode.SELECTION_HIDE_PANEL);
			moveSelectionCursorTo(cursor, x, y - headerHeight);
			return true;
		}
	...
	public boolean onFingerMove(int x, int y) {
        ...
        // TODO
        myReader.runAction(ActionCode.HIDE_BARS);
...
//			context.drawLine(gaugeRight, lineWidth, left, lineWidth);
            ...
            final ZLColor fillBgColor = myViewOptions.getColorProfile().FooterFillBgOption.getValue();
            context.setFillColor(fillBgColor);
            context.fillRectangle(left, height - 2 * lineWidth, right, lineWidth + 1);

            final int gaugeInternalRight =
				left + lineWidth + (int)(1.0 * myGaugeWidth * pagePosition.Current / pagePosition.Total);

			context.setFillColor(fillColor);
			context.fillRectangle(left, height - 2 * lineWidth, gaugeInternalRight, lineWidth + 1);
    ...
    private class Header implements HeaderArea {
        ...
        public int getHeight() {
            return myViewOptions.HeaderHeight.getValue();
        }
        ...
        public synchronized void paint(ZLPaintContext context) {
            ...
            final int fontSize = height > 50 ? height / 3 : height + 1;
            context.setFont(
                    myFontEntry,
                    fontSize,
                    false, false, false, false
            );
//            context.setFont(myFontEntry, 50, false, false, false, false);
            ...
            final int delta = (height - fontSize) / 3;

            // draw info text
            context.setTextColor(fgColor);
            context.drawString(right - infoWidth, height - delta, infoString);

            TOCTree tocElement = myReader.getCurrentTOCElement();
            if (tocElement != null) {
                String tocText = tocElement.getText();
                TextPaint textPaint = new TextPaint();
                textPaint.setTextSize(fontSize);
                float textWidth = context.getStringWidth(tocText);
                if (left + textWidth + infoWidth > right) { // 截断
                    float exceedSize = left + textWidth + infoWidth - right; // 超出多少
                    int size = (int) (exceedSize / fontSize);
                    tocText = tocText.substring(0, tocText.length() - size - 2);
                    tocText = tocText + "...";
                }
                context.drawString(left, height - delta, tocText.toCharArray(), 0, tocText.length());
            }

            // draw under line
            // int x0, int y0, int x1, int y1
            int lineHeight = height / 40;
            final ZLColor dividerBgColor = myViewOptions.getColorProfile().HeaderDividerOption.getValue();
            context.setFillColor(dividerBgColor);
            context.fillRectangle(left, height - lineHeight, right, height);
        }
    ...
    public HeaderArea getHeaderArea() {
        if (myViewOptions.ScrollbarType.getValue() == SCROLLBAR_SHOW_AS_FOOTER) {
            if (myHeader == null) {
                myHeader = new Header();
                myReader.addTimerTask(myHeader.UpdateTask, 15000);
            }
        } else {
            if (myHeader != null) {
                myReader.removeTimerTask(myHeader.UpdateTask);
                myHeader = null;
            }
        }
        return myHeader;
