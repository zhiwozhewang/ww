...
public class MyMagazineDirectoryAdapter extends BaseExpandableListAdapter {
    private List<GroupListData> gList;
    private Context mContext;
    private List<String> childIsReadList;

    public MyMagazineDirectoryAdapter(Context context,
    ...
    public View getChildView(int groupPosition, int childPosition,
        ...
        if (childIsReadList != null) {
            for (int i = 0; i < childIsReadList.size(); i++) {
                String isRead = childIsReadList.get(i);
                if (isRead.equals(gList.get(groupPosition).getList()
                        .get(childPosition).getTitleID())) {
                    cHolder.ctv.setTextColor(Color.parseColor("#aaaaaa"));
                }
            }
        }
        return convertView;
